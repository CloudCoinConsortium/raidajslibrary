...
an: ["00000000000000000000000000000000", "00000000000000000000000000000000", "00000000000000000000000000000000", "1a40be3f128fef6b71e05b96b59783fa", "8fdc9689c0075496b29d2aeacfd66a80",
  ...
  it('createrecord: send/self with 3 raida down (code 0x0)', async function(){
  params = {"coin": coin, "amount": amount, "event_code": event_codes[0]}

  r = await raidajs.apiCreateRecord(params, callback3Rdown)
  expect(r.code).to.equal(0x0);
  test_guids[0] = r.guid
  })
  ...
  params = {"coin": coin, "guid": test_guids[0]}
  r = await raidajs.apiDeleteRecord(params, callback3Rdown)
  expect(r.code).to.equal(0x0);
  })

  it('createrecord: other_know/receive with fracked coin (code 0x0)', async function(){
  params = {"coin": fracked, "amount": amount, "event_code": event_codes[1]}
  r = await raidajs.apiCreateRecord(params)
  expect(r.code).to.equal(0x0);
  test_guids[1] = r.guid
})
